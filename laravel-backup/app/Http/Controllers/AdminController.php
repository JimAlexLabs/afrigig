...
class AdminController extends Controller
{
    ...
    {
        $payments = Payment::with(['user', 'job'])->latest()->paginate(10);
    ...
    public function dashboard()
    {
        ...
        // Calculate total jobs and growth
        $totalJobs = Job::count();
        $jobsThisMonth = Job::where('created_at', '>=', $monthStart)->count();
        $jobsLastMonth = Job::whereBetween('created_at', [$lastMonthStart, $monthStart])->count();
        $jobGrowth = $jobsLastMonth > 0 ? (($jobsThisMonth - $jobsLastMonth) / $jobsLastMonth) * 100 : 0;

        // Calculate active bids
        $activeBids = Bid::where('status', 'pending')->count();
        ...
        $earningsThisMonth = Payment::where('status', 'completed')
            ->where('created_at', '>=', $monthStart)
            ->sum('amount');
        $earningsLastMonth = Payment::where('status', 'completed')
