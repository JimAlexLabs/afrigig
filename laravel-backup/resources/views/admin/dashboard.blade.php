    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white rounded-lg shadow-lg overflow-hidden transform transition-all duration-300 hover:scale-105">
                <div class="p-6">
                    <div class="flex items-center">
                        ...
                        <div class="ml-4">
                            <h3 class="text-2xl font-bold text-gray-900">{{ number_format($totalUsers ?? 0) }}</h3>
            ...
            <div class="bg-white rounded-lg shadow-lg overflow-hidden transform transition-all duration-300 hover:scale-105">
                <div class="p-6">
                    <div class="flex items-center">
                        ...
                        <div class="ml-4">
                            <h3 class="text-2xl font-bold text-gray-900">{{ number_format($activeJobs ?? 0) }}</h3>
            ...
            <div class="bg-white rounded-lg shadow-lg overflow-hidden transform transition-all duration-300 hover:scale-105">
                <div class="p-6">
                    <div class="flex items-center">
                        ...
                        <div class="ml-4">
                            <h3 class="text-2xl font-bold text-gray-900">${{ number_format($totalEarnings ?? 0, 2) }}</h3>
                            <p class="text-gray-600">Monthly Earnings</p>
                        </div>
                    </div>
                    <div class="mt-4">
            ...
            <div class="bg-white rounded-lg shadow-lg overflow-hidden transform transition-all duration-300 hover:scale-105">
                <div class="p-6">
                    <div class="flex items-center">
                        ...
                        <div class="ml-4">
                            <h3 class="text-2xl font-bold text-gray-900">{{ number_format($pendingAssessments ?? 0) }}</h3>
                            <p class="text-gray-600">Pending Assessments</p>
                        </div>
                    </div>
                    <div class="mt-4">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div class="bg-white rounded-lg shadow-lg overflow-hidden">
                <div class="p-6">
                    <div class="flex justify-between items-center mb-4">
                        ...
                        <a href="{{ route('admin.jobs.index') }}" class="text-blue-600 hover:text-blue-700">View all</a>
                    </div>
                    <div class="space-y-4">
                        @forelse($recentJobs ?? [] as $job)
                        <div class="flex items-center justify-between p-4 bg-gray-50 rounded-lg transform transition-all duration-300 hover:scale-102">
                            <div>
                                <h4 class="font-medium text-gray-900">{{ $job->title }}</h4>
                                <p class="text-sm text-gray-600">Posted {{ $job->created_at->diffForHumans() }}</p>
                            </div>
                            <span class="px-3 py-1 text-xs font-medium rounded-full {{ $job->status === 'open' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800' }}">
                                {{ ucfirst($job->status) }}
                            </span>
                        </div>
                        @empty
                        <p class="text-gray-600 text-center py-4">No recent jobs found.</p>
                        @endforelse
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-lg shadow-lg overflow-hidden">
                <div class="p-6">
                    <div class="flex justify-between items-center mb-4">
                        <h3 class="text-xl font-bold text-gray-900">Recent Assessments</h3>
                        <a href="{{ route('admin.skill-assessments.index') }}" class="text-blue-600 hover:text-blue-700">View all</a>
                    </div>
                    <div class="space-y-4">
                        ...
                        <div class="flex items-center justify-between p-4 bg-gray-50 rounded-lg transform transition-all duration-300 hover:scale-102">
                            <div>
                                ...
                                <p class="text-sm text-gray-600">Created {{ $assessment->created_at->diffForHumans() }}</p>
                            </div>
                            <span class="px-3 py-1 text-xs font-medium rounded-full {{ $assessment->status === 'completed' ? 'bg-blue-100 text-blue-800' : 'bg-yellow-100 text-yellow-800' }}">
                                {{ ucfirst($assessment->status) }}
                            </span>
                        </div>
                        @empty
                        <p class="text-gray-600 text-center py-4">No recent assessments found.</p>
                        @endforelse
                    </div>
                </div>
            </div>
